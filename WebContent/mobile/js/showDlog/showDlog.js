...
ShowDlog.prototype.mobileConfirm=function(setting){
	var icon="<span class='dlog-icon-alert'></span>";
	if(setting.icon && setting.icon=="success"){
		icon="<span class='dlog-icon-success'></span>";
	}else if(setting.icon && setting.icon=="error"){
	...
	if(setting.className){
		customCss=setting.className;
	}
	this.showDialog(
		"<div class='confirm-content'>" +
		"<p class='mb-dlog-alert-msg'>"+icon+" "+setting.msg+"</p>" +
		"<div class='mb-dlog-alert-btn-bar'>" +
		"<span class='mb-dlog-btn-cancel' onclick='$dlog.close(\"cancel\")'>取消</span>" +
		"<span class='mb-dlog-btn-ok' onclick='$dlog.close(\"ok\")'>确认</span>" +
		"</div>" +
		"</div>", "确认提示", function(cmd){
		if(cmd=="ok"){
			if(setting && setting.ok)
