...
function excelExport(tableid) {//整个表格拷贝到EXCEL中
	if(getExplorer()=='ie')
	{
...
var tableToExcel = (function() {
    ...
    return function(table, name) {
        ...
        var tempTab = $("<table>"+table.innerHTML+"</table>");
		tempTab.find(".noExport").remove();//.replace(/<img.*\/>/g,'').replace(/<img.*>/g,'')
//		alert(tempTab.html());
        var ctx = {
            worksheet: name || 'Worksheet',
            table: tempTab[0].innerHTML
        };//alert(ctx.table);
